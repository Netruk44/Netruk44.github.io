---
title: "EV Charging Calculator"
date: 2025-11-20T16:21:24-05:00
draft: false
tags: ["iOS", "Swift", "Apple"]
summary: "EV Charging Calculator is the simplest way to estimate how long it will take to charge your electric vehicle."
params:
  hideList: true
---

{{< storage-img src="./app_icon.png" center=true />}}
{{% img-subtitle %}}
*App icon*
{{% /img-subtitle %}}

EV Charging Calculator is the simplest way to estimate how long it will take to charge your electric vehicle. Whether you're plugging into a standard wall outlet, a Level 2 home charger, or a DC Fast Charger, get instant answers on when you can get back on the road.

### Key Features
*   **Instant Estimates:** Just enter your current battery % and target %, and see exactly how long charging will take.
*   **Smart Finish Time:** Know precisely when your car will be ready ("Tomorrow at 7:30 AM").
*   **Vehicle Profiles:** Save multiple cars with different battery sizes. Perfect for multi-EV households!
*   **Global Voltage Support:** Built-in presets for North American (120V/240V) and International (230V) voltages.
*   **Customizable Chargers:** Select from common presets (Level 1, Level 2, DCFC) or enter a custom kW power rate.
*   **Simple & Clean:** A distraction-free interface designed for quick calculations.

> ## Support Requests
>
> For support, please send me an e-mail at <a href="mailto:evchargerapp@danieltperry.me?subject=EV Charge Calculator Support">evchargerapp\@danieltperry.me</a>
> 
> If you have any suggestions or feedback, feel free to send me an e-mail at the same address.

# Info
* **Status**: Released
* **State**: Full release.
* **Download**: Available in the [App Store](https://apps.apple.com/us/app/ev-charging-calculator/id6755452503).
* **Privacy Policy**: [View Privacy Policy](./privacy)
* **Technologies & Languages**: Swift, SwiftUI

## Related Posts and Projects
- **Blog Post**: [How I Made My Todo List Voting Feature](/post/how-i-made-my-todo-list-voting-feature/)
- 